...
class SimConvNet(nn.Module):
    def __init__(self, p,m,n,f,w,h):
        super().__init__()
        self.com_base =  SimConvNetPart1(p,m,n,f,w,h)
        self.dist0_linear = nn.Linear(self.com_base.policy_output_size, p)
    def forward(self, state_input):
        _, action_masks = state_input
        out, x_val = self.com_base(state_input)
        x_act = self.dist0_linear(out) + (action_masks+1e-50).log()
        x_act = F.log_softmax(x_act)
        return x_act, x_val
        
class SimConvNetPart1(nn.Module):
    def __init__(self, p,m,n,f,w,h):
        super().__init__()
        num_inputs = n*f
        input_size = w*h
...
#        self.dist0_linear = nn.Linear(self.policy_output_size, p)
...
class PolicyValueNet:
    ...
    def __init__(self, model_file=None, use_gpu=False, is_train = True):
        ...
        if self.use_gpu:
            self.policy_value_net = SimConvNet(self.p, self.m, self.n, self.f, self.w, self.h).cuda()
        else:
            self.policy_value_net = SimConvNet(self.p, self.m, self.n, self.f, self.w, self.h)
    ...
    def load_pretrained_weight(self, path, load_way='ori'):
        ...
        model_name = self.policy_value_net.__class__.__name__
        if model_name == 'Net':
            ...
            self.policy_value_net.com_base.load_state_dict(weight_dict['com_base'])
            self.policy_value_net.dist0_linear.load_state_dict(weight_dict['dist0_fc'])

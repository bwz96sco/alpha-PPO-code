...
class MCTS(object):
    ...
    def _evaluate_random_rollout(self, state, limit=1000):
        ...
        for i in range(limit):
            ...
            max_action = np.random.choice(legal_actions)
    ...
    def _random_prob(self, state, K = None):
        # current_state = state.canvas_obs
        legal_actions = state.availables
        
        # _, action_mask = state.getActionMask()
        # probs, _ = self._policy(current_state, action_mask)
        # probs = np.random.rand(envConfig.partNum)
        # probs = probs[legal_actions]

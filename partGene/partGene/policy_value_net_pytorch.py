...
class Net(nn.Module):
    ...
    def __init__(self,N,p,m,n,f,w,h):
...
class PolicyValueNet:
    ...
    def __init__(self, model_file=None, use_gpu=False, is_train = True, resblock_num=9):
        ...
        self.p, self.m = obConfig.output_space, None#envConfig.partNum,envConfig.machNum
        self.n, self.f = obConfig.stackFrameNum,obConfig.feaNum
        self.w, self.h = obConfig.width, obConfig.height
        # the policy value net module
        if self.use_gpu:
            self.policy_value_net = Net(self.N,self.p, self.m, self.n, self.f, self.w, self.h).cuda()
        else:
            self.policy_value_net = Net(self.N,self.p, self.m, self.n, self.f, self.w, self.h)

...
class Game:
    ...
    def evaluate_rollout(self, obs_tuple):
        ...
        while 1:
            ...
            acts = self.transActsForPartGene(acts)
    ...
    def transActsForPartGene(self, acts):
        #transfer acts for partGene
        min_beam_size = min(self.dynamic_beam_size, self.beam_size)
        seq = self.board.sche.model_seq
        acts_dynamic = acts[:min_beam_size]
        acts_dynamic_abs = np.where(seq == acts_dynamic[:,None])[-1]
        acts[:min_beam_size] = acts_dynamic_abs
        return acts

...
class ScheEnv(gym.Env):
    ...
    def init_board(self):
        ...
        self.sche.model_seq = np.random.choice(oc.output_space,self.partNum,\
                                           replace=False)
        self.sche.model_seq.sort()
    ...
    def step(self, action):
        action = int(action)
#        action = np.where(self.sche.model_seq==action)[0][0]#part-add
        if action == -1 or action in self.availables:
            self.sche.step(action)#mult-actions frame
            self.availables = self.sche.available()
        else:
            raise Exception('act is illegal')
    ...
    def getActionMask(self): #need change
        vObs = np.array([0],dtype=np.float)
#        partAvai = self.availables
        
        partAvai = self.sche.model_seq[self.availables]#part-add
        partMask = np.zeros(oc.output_space)
        partMask[partAvai] = 1
        return vObs,partMask
    
    def actObsInit(self):
        self.action_space = [
                gym.spaces.Discrete(oc.output_space),
                ]
